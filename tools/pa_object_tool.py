...
PA_OBJECT_SCHEMA = {
    "name": "pa_object",
    "description": (
        "Record or update one of Bill's canonical personal-assistant items, "
        "AFTER you have understood the request through normal conversation.\n\n"
        "Two kinds of item:\n"
        "- obligation: something Bill has to do, owes, or is waiting on.\n"
        "- needs_bill: a decision only Bill can make.\n\n"
        "WHEN TO CALL THIS TOOL:\n"
        "- Only when the meaning is sufficiently clear. If key details are "
        "missing or ambiguous (which match? who? by when? which existing item?), "
        "do NOT call the tool — ask Bill a natural follow-up question instead and "
        "wait for his answer. An incomplete request is a conversation, not a "
        "write.\n"
        "- Never invent a date, a person, a project, an option, or a decision. "
        "Pass a field only if Bill actually gave it. Leave it out otherwise.\n"
